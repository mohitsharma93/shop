...
const Register = ({ history }) => {
  ...
  const { isAuthenticated, error, logInInProcess } = useSelector(
    (state) => state.auth
  );
  ...
  const onChange = (e) => {
    if (e.target.name === 'avatar') {
      ...
      reader.onload = () => {
        // there is three state 0 => dead, 1 => in process , 2 => success,
      ...
      <div className="row wrapper">
        <div className="col-10 col-lg-5">
          <form className="shadow-lg" encType='multipart/form-data' onSubmit={submitHandler}>
            ...
            <button id="register_button" type="submit" className="btn btn-block py-3" disabled={logInInProcess ? true : false}>
              REGISTER
          </button>
          </form>
        </div>
